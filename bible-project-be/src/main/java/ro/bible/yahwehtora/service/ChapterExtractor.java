...
public class ChapterExtractor {
    ...
    public ChapterExtractor(String bookName, String chapterName, String chapterKeyName) {
...
//        writeReport(problematicVerses);
    ...
    private void writeReport(List<Tuple2<Integer, Integer>> problematicVerses) {
        if (!problematicVerses.isEmpty()) {
            ReportWriter reportWriter = new ReportWriter(BibleStringUtils.removeDiacritics(bookName).replaceAll(" ", "-") + "migration-report-");
            reportWriter.writeLine("#####################");
            reportWriter.writeLine("Date '" + reportWriter.getDateNowFormatted() + "'");
            reportWriter.writeLine("Report for book '" + bookName + "'");
            reportWriter.writeLine("Chapter '" + chapterName + "'");
            reportWriter.writeLine("");

            problematicVerses.forEach(tuple -> {
                reportWriter.writeLine("#####");
                reportWriter.writeLine("Local HTML verseNo: '" + tuple.getItem1() + "' but sequence no is: '" + tuple.getItem2() + "' - Will add the sequence no in the db");
            });
            reportWriter.writeLine("##########-Report end-###########");
        }
    }
