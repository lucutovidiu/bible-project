...
export class ResultDisplayFullChapterComponent implements OnInit, OnDestroy {
  ...
  protected readonly resultDisplayPageLoading$ = this.fullChapterDisplayService.resultDisplayPageLoading$;

  constructor(private readonly fullChapterDisplayService: FullChapterDisplayService,
  ...
  private findChapterNumberByBook() {
    if (this.selectedBook && this.selectedBook.chapterNumber && this.selectedBook.bookId) {
      this.fullChapterDisplayService.findOrRetrieveVerseByChapterNumberByBook(this.selectedBook.chapterNumber, this.selectedBook.bookId)
        .subscribe(bibleVerses => {
          this.bibleVerse$.next(bibleVerses);
          HtmlFunctions.jumpToSection("jump_section");
        })
    }
  }
